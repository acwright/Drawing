        ...
        ScrollView([.horizontal, .vertical], showsIndicators: false) {
            VStack(spacing: 0) {
                GeometryReader { geometry in
                    VStack(spacing: 1) {
                        HStack(spacing: 1) {
                            LayerObjectView(size: $pixels.size, pixels: $pixels.pixels[0])
                                .frame(width: self.width(), height: self.height())
                            LayerObjectView(size: $pixels.size, pixels: $pixels.pixels[1])
                                .frame(width: self.width(), height: self.height())
                        }
                        HStack(spacing: 1) {
                            LayerObjectView(size: $pixels.size, pixels: $pixels.pixels[2])
                                .frame(width: self.width(), height: self.height())
                            LayerObjectView(size: $pixels.size, pixels: $pixels.pixels[3])
                                .frame(width: self.width(), height: self.height())
                        }
                    }
